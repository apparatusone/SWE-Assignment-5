...
def create_order_detail(order_details_in: schemas.OrderDetailCreate, db: Session = Depends(get_db)):
    return order_details.create(db=db, order_details=order_details_in)
...
def read_one_order_detail(order_details_id: int, db: Session = Depends(get_db)):
    od = order_details.read_one(db, order_details_id=order_details_id)
    if od is None:
...
def update_order_detail(order_details_id: int, order_details_in: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    if order_details.read_one(db, order_details_id=order_details_id) is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
    return order_details.update(db=db, order_details_id=order_details_id, order_details=order_details_in)

@app.delete("/order_details/{order_details_id}", tags=["OrderDetails"])
def delete_order_detail(order_details_id: int, db: Session = Depends(get_db)):
    if order_details.read_one(db, order_details_id=order_details_id) is None:
        raise HTTPException(status_code=404, detail="OrderDetail not found")
...
def create_recipe(recipe: schemas.RecipeCreate, db: Session = Depends(get_db)):
    return recipes.create(db=db, recipe_details=recipe)
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    recipe_detail = recipes.read_one(db, recipe_details_id=recipe_id)
...
def update_one_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    existing = recipes.read_one(db, recipe_details_id=recipe_id)
    if existing is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.update(db=db, recipe_details_id=recipe_id, recipe_details=recipe)
...
def delete_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    ...
    if existing is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return recipes.delete(db=db, recipe_details_id=recipe_id)